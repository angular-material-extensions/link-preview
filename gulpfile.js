...
const config = {
  libraryName: '@angular-material-extensions/link-preview',
  unscopedLibraryName: 'link-preview',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
const dependencies = ['ngx-linkifyjs'];
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /link-preview/ --deploy-url /link-preview/`, {cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @angular-material-extensions/link-preview-demo:server`, {cwd: `${config.demoDir}`}))
